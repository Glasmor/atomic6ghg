...
from atomic6ghg.factors import (
    ...
    refrigeration_and_ac_equipment_emission_factors,
...
class RefrigerationAndAc(Formula):
    ...
    def make_material_balance(self):
        ...
        for row in self.wks_data.get("materialBalance", []):
            ...
            co2_equivalent_emissions = RefrigerationAndAc.calculate_co2_emissions_material_balance(
    ...
    def make_simplified_material_balance(self):
        ...
        for row in self.wks_data.get("simplifiedMaterialBalance", []):
            ...
            co2_equivalent_emissions = RefrigerationAndAc.calculate_co2_emissions_simplified_material_balance(
                gwp,
                new_units_charge,
                new_units_capacity,
    ...
    def make_screening_method(self):
        ...
        for row in self.wks_data.get("screeningMethod", []):
            ...
            co2_equivalent_emissions = RefrigerationAndAc.calculate_co2_emissions_screening_method(
                gwp, type_of_equipment, new_units_charge, capacity_operating_units, capacity_disposed_units
            )
    ...
    @staticmethod
    def calculate_co2_emissions_screening_method(
        gwp, type_of_equipment, new_units_charge, capacity_operating_units, capacity_disposed_units
    ):
        """Calculate CO2 emissions for a gas given simplified material balance inputs"""
        installation_emission_factor = refrigeration_and_ac_equipment_emission_factors[type_of_equipment][
            "installationEmissionFactor"
        ]
        operating_emission_factor = refrigeration_and_ac_equipment_emission_factors[type_of_equipment][
            "operatingEmissionFactor"
        ]
        refrigerant_remaining_at_disposal = refrigeration_and_ac_equipment_emission_factors[type_of_equipment][
            "refrigerantRemainingAtDisposal"
        ]
        recovery_efficiency = refrigeration_and_ac_equipment_emission_factors[type_of_equipment]["recoveryEfficiency"]

        ret = gwp * (
            new_units_charge * installation_emission_factor
