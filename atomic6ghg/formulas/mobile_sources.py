...
from atomic6ghg.factors import (
    mobile_combustion_co2_emission_factors,
    mobile_combustion_ch4_and_n2o_emission_factors,
...
class MobileSources(Formula):
    ...
    co2_fuels_units = {
        "gasoline": "gallons",
        "diesel": "gallons",
        "residualFuelOil": "gallons",
        "aviationGasoline": "gallons",
        "jetFuel": "gallons",
        "lpg": "gallons",
        "ethanol": "gallons",
        "biodiesel": "gallons",
    ...
    def recalc(self, wks_data: dict) -> dict:
        ...
        self._total_emissions = {"CO2": 0.0, "CH4": 0.0, "N2O": 0.0}

        self.total_fuel_usage_and_co2_emissions = {
            fuel_type: {"fuelUsage": 0, "CO2": 0.0} for fuel_type in self.co2_fuels_units
        }
    ...
    def tabulate_subtable_data(self):
        ...
        for row in self.wks_data.get("mobileSourcesFuelConsumption", []):
            ...
            if vehicle_type:
                self.tabulate_mobile_source_use_and_emissions_by_vehicle_type(
                    vehicle_type, fuel_type, vehicle_year, fuel_usage, miles_traveled
                )
    ...
    def tabulate_mobile_source_use_and_emissions_by_vehicle_type(
        self, vehicle_type, fuel_type, vehicle_year, fuel_usage, miles_traveled
    ):
        ...
        year_display = mobile_combustion_ch4_and_n2o_emission_factors[vehicle_type][fuel_type][vehicle_year]["year_display"]
    ...
    def calculate_ch4_emissions(vehicle_type, fuel_type, usage, vehicle_year):
        ...
        ret = mobile_combustion_ch4_and_n2o_emission_factors[vehicle_type][fuel_type][vehicle_year]["ch4_factor"] * usage
        return ret
    ...
    def calculate_n2o_emissions(vehicle_type, fuel_type, usage, vehicle_year):
        """Calculate CH4 emissions for a mobile source vehicle. usage is either fuel_usage or miles_traveled depending
        on if the vehicle_type is non-road or on road, respectively."""
        ret = mobile_combustion_ch4_and_n2o_emission_factors[vehicle_type][fuel_type][vehicle_year]["n2o_factor"] * usage
        return ret

    def make_total_co2_equivalent_emissions(self):
        """Calculate total CO2 equivalent emissions"""
        total = (
            self._total_emissions["CO2"]
            + refrigerants_gwp_factors["ch4"] * self._total_emissions["CH4"] / 1000.0
            + refrigerants_gwp_factors["n2o"] * self._total_emissions["N2O"] / 1000.0
        ) / 1000.0
    ...
    def make_total_biomass_co2_equivalent_emissions(self):
        ...
        total = (
            self.total_fuel_usage_and_co2_emissions["ethanol"]["fuelUsage"]
            * self.ethanol_percent
            / 100.0
            * mobile_combustion_co2_emission_factors["ethanol"]
            + self.total_fuel_usage_and_co2_emissions["biodiesel"]["fuelUsage"]
            * self.biodiesel_percent
            / 100.0
            * mobile_combustion_co2_emission_factors["biodiesel"]
        ) / 1000.0
