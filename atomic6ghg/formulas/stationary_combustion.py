...
class StationaryCombustion(Formula):
    ...
    common_units = {
        "anthraciteCoal": "shortTon",
        "bituminousCoal": "shortTon",
        "distillateFuelOilNo2": "gallons",
        "kerosene": "gallons",
        "landfillGas": "scf",
        "ligniteCoal": "shortTon",
        "liquefiedPetroleumGases": "gallons",
        "naturalGas": "scf",
        "residualFuelOilNo6": "gallons",
        "subBituminousCoal": "shortTon",
    ...
    def make_total_combustion(self):
        ...
        total_combustion = {fuel: 0.0 for fuel in self.all_fuels}
        ...
        total_combustion = [
            {"fuelType": fuel, "quantityCombusted": quantity_combusted, "units": self.common_units[fuel]}
            for fuel, quantity_combusted in total_combustion.items()
        ]
    ...
    def make_emissions(self):
        ...
        emissions = {fuel: {"CO2": 0.0, "CH4": 0.0, "N2O": 0.0} for fuel in self.all_fuels}
        emissions["totalFossilFuelEmissions"] = {"CO2": 0.0, "CH4": 0.0, "N2O": 0.0}
        emissions["totalNonFossilFuelEmissions"] = {"CO2": 0.0, "CH4": 0.0, "N2O": 0.0}
        ...
        emissions["totalEmissionsForAllFuels"] = {
            gas: emissions["totalFossilFuelEmissions"][gas] + emissions["totalNonFossilFuelEmissions"][gas]
            for gas in emissions["totalFossilFuelEmissions"]
        }

        self._total_emissions = {
            "totalFossilFuelEmissions": emissions["totalFossilFuelEmissions"],
    ...
    def make_co2_equivalent_emissions(self):
        ...
        conversions = {"CO2": 1.0, "CH4": 1 / 1000.0, "N2O": 1 / 1000.0}
        total_fossil = sum(
            [
                self._total_emissions["totalFossilFuelEmissions"][gas]
                * refrigerants_gwp_factors[gas.lower()]
                * conversions[gas]
        ...
        total_nonfossil = sum(
            [
                self._total_emissions["totalNonFossilFuelEmissions"][gas]
                * refrigerants_gwp_factors[gas.lower()]
                * conversions[gas]
                for gas in self._total_emissions["totalNonFossilFuelEmissions"]
                if gas != "CO2"
            ]
        )
        total = (total_fossil + total_nonfossil) / 1000.0
    ...
    def make_biomass_co2_equivalent_emissions(self):
        ...
        total = self._total_emissions["totalNonFossilFuelEmissions"]["CO2"] / 1000.0
