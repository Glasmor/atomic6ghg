...
class WasteGases(Formula):
    ...
    def recalc(self, wks_data):
        ...
        self.gas_total_number_of_moles_per_unit_volume = (
            self.wks_data.get("gasTotalNumberOfMolesPerUnitVolume")
            if self.wks_data.get("gasTotalNumberOfMolesPerUnitVolume")
            else self.default_gas_total_number_of_moles_per_unit_volume
        )
        self.oxidation_factor = (
            self.wks_data.get("oxidationFactor") if self.wks_data.get("oxidationFactor") else self.default_oxidation_factor
        )
    ...
    def make_emission_factor_for_gas_waste_stream(self):
        ...
        waste_gasses = []
        self._total_emissions = {"totalCarbonContent": 0.0, "totalMoles": 0.0, "totalMolarFraction": 0.0}
    ...
    def make_total_all_components(self):
        ...
        total_all_components = {
            "molarFraction": self._total_emissions["totalMolarFraction"],
    ...
    def make_co2_equivalent_emissions(self):
        ...
        if waste_stream_gas_combusted:
            total = (
                self._total_emissions["totalCarbonContent"]
                * self.oxidation_factor
                / 100.0
                * (44.0098 / 12.001)
                * waste_stream_gas_combusted
